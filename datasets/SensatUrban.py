...
class SensatUrbanDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True, colab=False):
        ...
        if colab:
            ...
            self.additional_suffix = "training" if set == "validation" else set
        else:
            self.path = '/home/jelena/datasets/sensat_urban/'
            self.path_processing = '/home/jelena/datasets/sensat_urban/'
            self.additional_suffix = ""
        ...
        self.files = []
        self.cloud_names = self.train_val_test_split_files.get(self.set, None)
        if self.cloud_names is None:
            raise ValueError("Unknown set for Sensat Urban data")
        for cloud_name in self.cloud_names:
            self.files.append(join(self.path, self.additional_suffix + cloud_name + ".ply"))
    ...
    def load_subsampled_clouds(self):
        ...
        tree_path = join(self.path_processing, 'input_{:.3f}'.format(dl))
